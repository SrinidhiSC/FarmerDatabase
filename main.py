...
@login_required
def orderss():
    query = db.session.query(orders.oid, orders.pid, Addagroproducts.productname, Addagroproducts.price, Addagroproducts.username).join(Addagroproducts, orders.pid == Addagroproducts.pid).filter( orders.id ==current_user.id, Addagroproducts.pid == orders.pid).all()
    return render_template('orders.html',query=query)

@app.route('/orders/<int:pid>', methods=['POST','GET'])
@login_required
def purchase(pid):
    if request.method == "POST":
        product = Addagroproducts.query.get(pid)
        oid = randint(100000, 999999)  # You can adjust the range as needed
        id = current_user.id
        order = orders(oid=oid, id=id, pid=product.pid)
